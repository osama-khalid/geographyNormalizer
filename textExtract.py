...
with open(path,'r', encoding="utf-8") as csvfile:   
    ...
    for row in readCSV: 
        ...
        flag=0
        if len(row)>7:
            R=row[7].replace('\t',' ').replace('[','').replace(']','')
            if R.find('None')==-1:
                
                loc=R.strip(' ').split(',')
                if len(loc)>1:
                    try:
                        location = geolocator.reverse(str(loc[1])+","+str(loc[0]))
                        if'address' in location.raw:
                            geoLoc=location.raw['address']
                            if 'city' in geoLoc:
                                city=geoLoc['city']
